...
describe("session module", () => {
  ...
  describe("session.fromPartition(partition, options)", () => {
    it("returns existing session with same partition", () => {
      expect(session.fromPartition("test")).to.equal(
        session.fromPartition("test")
      );
    });
  });
  ...
  describe("ses.cookies", () => {
    ...
    it("fails to set cookies with samesite=garbage", async () => {
      ...
      const value = "hithere";
      await expect(
        cookies.set({ url, value, sameSite: "garbage" as any })
    ...
    it("gets cookies without url", async () => {
      ...
      expect(cs.some((c) => c.name === name && c.value === value)).to.equal(
        true
      );
    });
    ...
    it("should overwrite previous cookies", async () => {
      ...
      for (const value of ["No", "Yes"]) {
        ...
        expect(
          list.some((cookie) => cookie.name === name && cookie.value === value)
        ).to.equal(true);
      }
    });
    ...
    it("emits a changed event when a cookie is added or removed", async () => {
      ...
      await cookies.remove(url, name);
      const [, removeEventCookie, removeEventCause, removeEventRemoved] =
        await b;
  ...
  describe("ses.clearStorageData(options)", () => {
    ...
    it("clears localstorage data", async () => {
      ...
      while (
        (await w.webContents.executeJavaScript("localStorage.length")) !== 0
      ) {
  ...
  describe("ses.protocol", () => {
    ...
    beforeEach(async () => {
      ...
      await customSession.protocol.registerStringProtocol(
        protocolName,
        handler
      );
    });
  ...
  describe("ses.setProxy(options)", () => {
    ...
    it("disallows configuring proxy settings with mode `invalid`", async () => {
      ...
      ).to.eventually.be.rejectedWith(/Invalid mode/);
    });
  ...
  describe("ses.setCertificateVerifyProc(callback)", () => {
    ...
    it("accepts the request when the callback is called with 0", async () => {
      ...
      ses.setCertificateVerifyProc(
        ({ hostname, verificationResult, errorCode }, callback) => {
    ...
    it("rejects the request when the callback is called with -2", async () => {
      ...
      ses.setCertificateVerifyProc(
        (
          { hostname, certificate, verificationResult, isIssuedByKnownRoot },
          callback
        ) => {
          if (hostname !== "127.0.0.1") return callback(-3);
          validate = () => {
            ...
            expect(certificate.issuerCert.issuer.commonName).to.equal(
              "Root CA"
            );
            expect(certificate.issuerCert.subject.commonName).to.equal(
              "Intermediate CA"
            );
            expect(
              certificate.issuerCert.issuerCert.issuer.commonName
            ).to.equal("Root CA");
            expect(
              certificate.issuerCert.issuerCert.subject.commonName
            ).to.equal("Root CA");
            expect(certificate.issuerCert.issuerCert.issuerCert).to.equal(
              undefined
            );
            expect(verificationResult).to.be.oneOf([
              "net::ERR_CERT_AUTHORITY_INVALID",
      ...
      await expect(w.loadURL(serverUrl)).to.eventually.be.rejectedWith(
        /ERR_FAILED/
      );
      expect(validate!).not.to.be.undefined();
      validate!();
    ...
    it("saves cached results", async () => {
      ...
      await expect(
        w.loadURL(serverUrl),
        "first load"
      ).to.eventually.be.rejectedWith(/ERR_FAILED/);
      await once(w.webContents, "did-stop-loading");
      await expect(
        w.loadURL(serverUrl + "/test"),
        "second load"
      ).to.eventually.be.rejectedWith(/ERR_FAILED/);
  ...
  describe("ses.clearAuthCache()", () => {
    it("can clear http auth info from cache", async () => {
      ...
      expect(await fetch(`http://test:test@127.0.0.1:${port}`)).to.equal(
        "authenticated"
      );
  ...
  // FIX: setSaveDialogOptions() method in the Electron DownloadItem class.
  ...
  describe("DownloadItem", () => {
    ...
    before(async () => {
      downloadServer = http.createServer((req, res) => {
        res.writeHead(200, {
          ...
          "Content-Disposition":
            req.url === "/?testFilename" ? "inline" : contentDisposition,
        });
        res.end(mockPDF);
      });
    ...
    it('pauses the download until the user has made a decision in the save dialog', async () => {
      const w = new BrowserWindow({ show: false });
      let downloadPromise;
      ...
      w.loadURL(`${url}:${port}/download`);
  
      // Show the save dialog synchronously to block the event loop
      ...
      // If the user canceled the dialog, cancel the download
      if (result === undefined) {
        w.webContents.session.cancelDownload(downloadPromise);
      } else {
        // Set the save path and resume the download
        w.webContents.session.setSavePath(downloadPromise, result);
      }
    ...
    describe("session.downloadURL", () => {
      ...
      it("can perform a download with a valid auth header", async () => {
        ...
        const downloadDone: Promise<Electron.DownloadItem> = new Promise(
          (resolve) => {
      ...
      it("correctly handles a download with an invalid auth header", async () => {
        ...
        const downloadFailed: Promise<Electron.DownloadItem> = new Promise(
          (resolve) => {
    ...
    describe("webContents.downloadURL", () => {
      ...
      describe("when a save path is specified and the URL is unavailable", () => {
        it("does not display a save dialog and reports the done state as interrupted", (done) => {
          ...
          w.webContents.downloadURL(
            `file://${path.join(__dirname, "does-not-exist.txt")}`
          );
        });
      });
    });
    ...
    describe("WebView.downloadURL", () => {
      it("can perform a download", async () => {
        ...
        const done: Promise<[string, Electron.DownloadItem]> = new Promise(
          (resolve) => {
            w.webContents.session.once("will-download", function (e, item) {
              item.savePath = downloadFilePath;
              item.on("done", function (e, state) {
        ...
        await w.webContents.executeJavaScript(
          `(${webviewDownload})(${JSON.stringify({ fixtures, url, port })})`
        );
  ...
  describe("ses.createInterruptedDownload(options)", () => {
    ...
    it("can be resumed", async () => {
      ...
      try {
        ...
        const w = new BrowserWindow({ show: false });
        const downloadCancelled: Promise<Electron.DownloadItem> = new Promise(
          (resolve) => {
            w.webContents.session.once("will-download", function (e, item) {
        ...
        const downloadResumed: Promise<Electron.DownloadItem> = new Promise(
          (resolve) => {
            w.webContents.session.once("will-download", function (e, item) {
              expect(item.getState()).to.equal("interrupted");
              item.setSavePath(downloadFilePath);
  ...
  describe("ses.setPermissionCheckHandler(handler)", () => {
    ...
    it("details provides requestingURL for cross origin subFrame", async () => {
      ...
      w.webContents.executeJavaScript(`
        ...
        null;
      `);
      const [, , frameProcessId, frameRoutingId] = await once(
        w.webContents,
  ...
  describe("session-created event", () => {
    it("is emitted when a session is created", async () => {
      const sessionCreated = once(app, "session-created") as Promise<
        [any, Session]
      >;
  ...
  describe("session.storagePage", () => {
    ...
    it("returns different paths for partitions and the default session", () => {
      expect(session.defaultSession.storagePath).to.not.equal(
  ...
  describe("session.setCodeCachePath()", () => {
    it("throws when relative or empty path is provided", () => {
      ...
      expect(() => {
        session.defaultSession.setCodeCachePath(
          path.join(app.getPath("userData"), "electron-test-code-cache")
        );
      }).to.not.throw();
    });
  });
  ...
  describe("ses.clearData()", () => {
    ...
    it("clears localstorage data", async () => {
      ...
      expect(
        await w.webContents.executeJavaScript("localStorage.length")
      ).to.be.greaterThan(0);
